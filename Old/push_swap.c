...
typedef struct s_stack
{
	...
	int		current_size;
	int		max_size;
	int offset; // Indica l'orientamento della stack,
		basato su quante volte è stata chiamata una funzione r* su questa)
	char	name;
}			t_stack;
...
int	stack_init(t_stack *stack, char name, int size, int minstep, int maxstep,
		int shuffles)
{
	int	i;
	int	content;
	...
	while (i < shuffles)
	{
		stack->content = array_swap_indexes(stack->content, random_int(0, size
					- 1), random_int(0, size - 1));
		i++;
	}
	return (1);
}
